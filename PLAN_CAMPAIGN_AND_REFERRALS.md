...
**Bundle context for Phase A:**

| Bundle | Price | Who can issue passes? | Status |
|--------|-------|----------------------|--------|
| `resident-vat` | £0.99/mo | Yes (100 tokens/mo, pass costs 3) | Imminent launch — see `PLAN_RESIDENT_VAT_ROLLOUT.md` |
| `resident-pro` | £9.99/mo | Yes (100 tokens/mo, pass costs 3) | Existing, pass-gated (not actively onboarding) |
| `resident-pro-comp` | Free | Yes (100 tokens/mo, pass costs 3) | Existing, admin-issued to beta testers |

Phase A launches alongside `resident-vat` as the primary subscription tier. The campaign economics assume £0.99/mo subscribers issuing 3-token campaign passes.
...
2. Find user's token-bearing bundle (resident-vat, resident-pro, or resident-pro-comp)
...
- Applies to any subscription bundle (`resident-vat`, `resident-pro`)
...
Add below the existing "My Generated Passes" section. Visible only to users with a token-bearing bundle (resident-vat, resident-pro, or resident-pro-comp).
...
Phase A launches with `resident-vat` at £0.99/mo. Token values scale with subscription price.

| Event | Reward | Value at £0.99/mo (resident-vat) | Value at £9.99/mo (resident-pro) |
|-------|--------|----------------------------------|----------------------------------|
| Pass redeemed | +2 tokens | ~£0.02 | ~£0.20 |
| Referred user submits first VAT return | +5 tokens (Phase B) | ~£0.05 | ~£0.50 |
| Referred user subscribes | +1 free month (Phase B) | ~£0.99 | ~£9.99 |
| **Commission** (after 3 conversions, Phase C) | 20% of first year | ~£2.38 per conversion | ~£23.98 per conversion |
...
Strategy: build the minimum viable referral system in-house using existing infrastructure + CLI/GitHub Actions for admin. Track the revenue generated by referrals. When referral-attributed subscription revenue exceeds ~£40/month (~$49), migrate tracking and payouts to FirstPromoter. This avoids paying for a platform before it's earning its keep.

### Upgrade trigger

With `resident-vat` at £0.99/mo, the breakeven for FirstPromoter ($49/mo ≈ £40/mo) is ~40 active referral-driven subscribers. With `resident-pro` at £9.99/mo, it's ~4 subscribers. The trigger is based on total referral-attributed revenue, not subscriber count.
...
Until then, scripts and DynamoDB queries are sufficient.
...
**Prerequisite**: `resident-vat` bundle must be fully rolled out (see `PLAN_RESIDENT_VAT_ROLLOUT.md`). Phase A launches with `resident-vat` subscribers (£0.99/mo) as the primary pass-issuing audience. `resident-pro` and `resident-pro-comp` users can also issue passes, but `resident-vat` is the bundle we are actively onboarding users to.

**Who can issue campaign passes**: Any user with a token-bearing bundle that includes the `generate-pass-digital` activity — currently `resident-vat`, `resident-pro-comp`, and `resident-pro` (see `submit.catalogue.toml` activities). This is enforced by the existing `passGeneratePost.js` entitlement check.
...
# Estimated monthly referral revenue: £0.99 (1 × resident-vat @ £0.99/mo)
# FirstPromoter breakeven: ~40 resident-vat subscribers (£40/mo) — currently at 1
```
...
|-------|---------|-------------|-------|-------------------|
| **A: Bootstrap** | `resident-vat` rollout complete | £0 | CLI scripts, manual token rewards | 0–20 |
| **B: Automate** | ~20 referrals, manual work tedious | £0 | Automated rewards, CLI reporting | 20–100 |
| **C: FirstPromoter** | ≥£40/mo referral-attributed revenue | ~£40/mo ($49) | Self-serve dashboard, automated payouts | 100+ |
...
*Last updated: 2026-03-16 — Updated Phase A to use resident-vat as primary subscription tier*
*GitHub Issue: #560*
