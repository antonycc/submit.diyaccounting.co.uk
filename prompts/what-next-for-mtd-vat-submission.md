...
- Use `package.json` scripts when proposing verification steps:
  ...
  - Behaviour/browser tests: `npm run test:behaviour` / `npm run test:behaviour-proxy` / `npm run test:browser`.
...
- Behaviour tests are very verbose — pipe output to a file when running (e.g. `npm run test:behaviour-proxy > target/behaviour-test-results/behaviour.log 2>&1`).
