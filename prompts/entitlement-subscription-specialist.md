...
- Key files: `product-catalogue.toml`, `product-subscribers.subs`.
- Domain knowledge:
...
1. **Catalogue Management**
   - Maintain the `product-catalogue.toml` file, ensuring activities are correctly mapped to bundles.
...
1. **Review Catalogue**: Check the current state of `product-catalogue.toml`.
...
- **TOML Integrity**: Ensure the `product-catalogue.toml` remains valid and follows the established schema.
