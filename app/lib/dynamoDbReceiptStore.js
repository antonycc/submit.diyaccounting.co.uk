...
function getTableName() {
  const tableName = process.env.DIY_SUBMIT_RECEIPTS_TABLE_NAME;
...
export async function putReceipt(userSub, receiptId, receipt) {
  if (!isDynamoDbEnabled()) {
    logger.warn({ message: "DynamoDB not enabled, skipping putReceipt" });
    return;
  } else {
    logger.info({ message: "DynamoDB enabled, proceeding with putReceipt" });
  }
...
export async function getReceipt(userSub, receiptId) {
  if (!isDynamoDbEnabled()) {
    logger.warn({ message: "DynamoDB not enabled, returning null" });
    return null;
  } else {
    logger.info({ message: "DynamoDB enabled, proceeding with getReceipt", userSub, receiptId });
  }
...
export async function listUserReceipts(userSub) {
  if (!isDynamoDbEnabled()) {
    logger.warn({ message: "DynamoDB not enabled, returning empty receipts array" });
    return [];
  } else {
    logger.info({ message: "DynamoDB enabled, proceeding with listUserReceipts", userSub });
  }
