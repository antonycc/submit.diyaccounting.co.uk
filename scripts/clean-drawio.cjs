...
const words = [
  'billing', 'checkout', 'portal', 'recover', 'webhook',
  'bundle', 'capacity', 'reconcile', 'delete', 'post', 'get', 'worker', 'zero', 'alias',
  'cognito', 'token', 'custom', 'authorizer',
  'hmrc', 'vat', 'obligation', 'return', 'receipt',
  'interest', 'pass', 'admin', 'generate', 'my', 'passes',
  'session', 'beacon',
  'activity', 'telegram', 'forwarder',
  'alert', 'topic', 'policy',
  'canary', 'artifacts', 'health',
  'api', 'http', 'domain', 'stage', 'default', 'mapping',
  'waitlist',
  'origin', 'bucket', 'web', 'dist',
  'doc', 'root', 'deployment', 'aws', 'cli', 'layer',
  'rate', 'limit', 'alarm', 'common', 'rule', 'bad', 'inputs',
  'cfn', 'stack', 'status',
  'self', 'destruct',
  'queue', 'dlq',
  'fn', 'role', 'service',
  'stripe', 'secret', 'key', 'test', 'price',
  'email', 'proof', 'state', 'change',
  'access', 'logs', 'ensure', 'log', 'group',
  'errors', 'throttles', 'high', 'duration', 'metric', 'filter',
  'record', 'upsert',
  'response', 'headers', 'request', 'prevention', 'fraud',
  'control', 'id', 'name',
  'provider', 'framework', 'event', 'on',
  'cleanup', 'gw',
  'objects', 'auto', 'resource', 'customer',
  'subscription', 'subscriptions',
  'schedule', 'destination', 'github', 'synthetic',
  'client', 'sandbox', 'bot',
  ...
  while (remaining.length > 0) {
    ...
    if (!matched) {
      ...
      while (remaining.length > 0) {
        let found = false;
        for (const w of words) {
          if (remaining.startsWith(w)) { found = true; break; }
        }
        if (found) break;
