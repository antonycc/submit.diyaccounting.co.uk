...
echo 'Starting DynamoDB Local...' >&2
# Stream to both console and log file
npm run dynamodb 2>&1 | tee "$DYNAMODB_LOG" &
BG_PIDS+=("$!")
...
echo 'Waiting for DynamoDB Local to report endpoint...' >&2
TEST_DYNAMODB_ENDPOINT=''

# Poll the log for the "DynamoDB started url=" line
for _ in {1..150}; do
  if grep -q 'DynamoDB started url=' "$DYNAMODB_LOG"; then
    TEST_DYNAMODB_ENDPOINT=$(awk -F'url=' '/DynamoDB started url=/{print $2; exit}' "$DYNAMODB_LOG")
    break
  fi
  sleep 0.2
done

if [[ -z "$TEST_DYNAMODB_ENDPOINT" ]]; then
  echo 'ERROR: Failed to detect DynamoDB endpoint in DynamoDB logs.' >&2
  echo "Contents of $DYNAMODB_LOG:" >&2
  sed -n '1,200p' "$DYNAMODB_LOG" >&2 || true
  exit 1
fi

echo "Detected DynamoDB endpoint: $TEST_DYNAMODB_ENDPOINT" >&2
...
TEST_S3_ENDPOINT="$TEST_S3_ENDPOINT" TEST_DYNAMODB_ENDPOINT="$TEST_DYNAMODB_ENDPOINT" npm run server
