...
function parseTOML(src) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (val.startsWith('"') && val.endsWith('"')) val = val.substring(1, val.length - 1);
      ...
      val = inner ? inner.split(',').map(function (v) {
        v = v.trim();
...
var oldFeatures = [
  'CashandBankFeature', 'CompanyFinalAccountsFeature', 'CompanyProfitandLossFeature',
  'CompanyPurchaseSpreadsheetFeature', 'CompanySalesSpreadsheetFeature', 'PayslipsFeature',
  'ProfitAndLossFeature', 'PurchaseSpreadsheetFeature', 'SECashandBankFeature',
  'SEProfitandLossFeature', 'SEPurchaseSpreadsheetFeature', 'SESalesSpreadsheetFeature',
  'SETaxReturnFeature', 'SEVATReturnFeature', 'SalesInvoiceFeature', 'SalesSpreadsheetFeature',
  'SelfAssessmentFeature', 'SelfEmployedTaxFeature', 'TaxandAssetsFeature', 'TaxiExpensesFeature',
  'TaxiIncomeTaxFeature', 'TaxiProfitandLossFeature', 'TaxiReceiptsFeature', 'TaxiSATaxFeature',
  'TaxiVATFeature', 'VatReturnsFeature', 'YearEndAccountsFeature',
];
...
sp.push('  <url><loc>https://spreadsheets.diyaccounting.co.uk/download.html</loc><changefreq>monthly</changefreq><priority>0.9</priority></url>');
sp.push('  <url><loc>https://spreadsheets.diyaccounting.co.uk/donate.html</loc><changefreq>yearly</changefreq><priority>0.5</priority></url>');
sp.push('  <url><loc>https://spreadsheets.diyaccounting.co.uk/knowledge-base.html</loc><changefreq>monthly</changefreq><priority>0.9</priority></url>');

// Product download pages
for (var prod of products) {
  sp.push('  <url><loc>https://spreadsheets.diyaccounting.co.uk/download.html?product=' + escapeXml(prod.id) + '</loc><priority>0.9</priority></url>');
}
