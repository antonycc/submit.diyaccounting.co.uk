...
//
// build-gateway-redirects.cjs — Generate CloudFront Function from redirects.toml
//
// Usage:
//   node scripts/build-gateway-redirects.cjs
//
// Reads:  web/www.diyaccounting.co.uk/redirects.toml
// Writes: web/www.diyaccounting.co.uk/redirect-function.js

const fs = require('fs');
const path = require('path');

const ROOT = path.resolve(__dirname, '..');
const TOML_PATH = path.join(ROOT, 'web', 'www.diyaccounting.co.uk', 'redirects.toml');
const OUTPUT_PATH = path.join(ROOT, 'web', 'www.diyaccounting.co.uk', 'redirect-function.js');
...
var functionCode = `// Auto-generated by scripts/build-gateway-redirects.cjs — do not edit
...
function handler(event) {
  ...
  if (uri.indexOf('/content/') === 0) {
    return {
      ...
      body: 'This API endpoint has been discontinued. Visit https://spreadsheets.diyaccounting.co.uk/ for current content.'
    };
  }
