...
// Gateway tests run against deployed CI or prod environments only.
// The gateway site is a separate CloudFront distribution from the submit site.
// Base URL is set via GATEWAY_BASE_URL env var, with CI default.
const gatewayBaseUrlRaw = process.env.GATEWAY_BASE_URL || "https://ci-gateway.diyaccounting.co.uk";
 ...
 *
 * These tests verify the gateway site (diyaccounting.co.uk) works correctly:
...
test.describe("Gateway Site - diyaccounting.co.uk", () => {
  ...
  test("About page loads and contains company information", async ({ page }) => {
    ...
    const headingText = await heading.textContent();
    expect(headingText).toContain("DIY Accounting Limited");
    console.log(" Main heading is correct");
  ...
  test("sitemap.xml is accessible and well-formed", async ({ page }) => {
    ...
    expect(content).toContain("diyaccounting.co.uk");
  ...
  test("Old www URL redirect patterns return 301", async ({ page }) => {
    addOnPageLogging(page);
