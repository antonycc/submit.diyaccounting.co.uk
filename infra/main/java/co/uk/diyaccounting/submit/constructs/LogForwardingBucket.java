...
public class LogForwardingBucket extends Stack {
  ...
  public static class Builder {
    ...
    public IBucket build() {
      ...
      logger.info(
          "Setting expiration period to %d days for %s".formatted(retentionPeriodDays, idPrefix));
      final IBucket logBucket =
          Bucket.Builder.create(scope, "%sLogBucket".formatted(idPrefix))
      ...
      if (handlerSource == null || handlerSource.isBlank() || "none".equals(handlerSource)) {
        ...
        final LogGroup logGroup =
            LogGroup.Builder.create(scope, "%sLogForwarderLogGroup".formatted(idPrefix))
                .logGroupName(functionName)
                .retention(RetentionDaysConverter.daysToRetentionDays(retentionPeriodDays))
                .removalPolicy(RemovalPolicy.DESTROY)
                .build();
        var logForwarderBuilder =
            Function.Builder.create(scope, "%sLogForwarder".formatted(idPrefix))
        ...
        logBucket.addEventNotification(
            EventType.OBJECT_CREATED, new LambdaDestination(logForwarder));
    ...
    private String buildFunctionName() {
      return functionNamePrefix != null
          ...
          : "%s-log-forwarder"
              .formatted(ResourceNameUtils.convertCamelCaseToDashSeparated(idPrefix));
    }
