...
public class MonitoringAlarms {
  ...
  private MonitoringAlarms(Builder builder) {
    ...
    this.alertTopic =
        Topic.Builder.create(builder.scope, "MonitoringAlerts")
    ...
    this.lambdaAlarms =
        builder.lambdaFunctions.stream()
            .map(
                lambda -> {
                  // Error rate alarm
                  Alarm errorAlarm =
                      Alarm.Builder.create(builder.scope, lambda.getFunctionName() + "ErrorAlarm")
                          .alarmName(lambda.getFunctionName() + "-ErrorRate")
                          .alarmDescription(
                              "High error rate for Lambda function " + lambda.getFunctionName())
                          .metric(
                  ...
                  Alarm durationAlarm =
                      Alarm.Builder.create(
                              builder.scope, lambda.getFunctionName() + "DurationAlarm")
                          .alarmName(lambda.getFunctionName() + "-Duration")
                          .alarmDescription(
                              "High duration for Lambda function " + lambda.getFunctionName())
                          .metric(
                              Metric.Builder.create()
                                  .namespace("AWS/Lambda")
                                  .metricName("Duration")
                ...
                bucket -> {
                  ...
                  Alarm storageAlarm =
                      Alarm.Builder.create(
                          ...
                          .alarmDescription(
                              "High storage usage for S3 bucket " + bucket.getBucketName())
                          .metric(
                              Metric.Builder.create()
                                  .namespace("AWS/S3")
