...
- Wire entitlementsService.getGrantedBundles to call the store (with a fallback to the in-memory store when TEST_BUNDLE_MOCK=true for local dev/tests).
...
- Mocks
    - Use aws-sdk-client-mock for DynamoDB and S3; feature flag TEST_BUNDLE_MOCK for in-memory mode.
...
- Feature flags:
    ...
    - TEST_BUNDLE_MOCK=true for local dev without AWS.
- Environment:
    - Provide DIY_SUBMIT_ENTITLEMENTS_TABLE (after DynamoDB added).
    - Continue to set TEST_S3_* for non-AWS S3 testing.
